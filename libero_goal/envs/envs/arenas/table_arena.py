...
class TableArenaExtended(TableArena):
    def set_camera(self, camera_name, pos, quat=None, camera_attribs=None):
        """
        set camera beyond pos and quat
        """
        ...
        #     breakpoint()
        if quat is None:
            if camera is not None and 'quat' in camera.attrib:
                del camera.attrib['quat']
        else:
            camera_attribs["quat"] = array_to_string(quat)
        # print(f'table_arena.py set_camera(): camera_name {camera_name} camera_attribs {camera_attribs}') # breakpoint()
        if camera is None:
