# Based on DynaMo (models/vq_behavior_transformer/bet.py): https://github.com/jeffacce/dynamo_ssl/blob/main/models/vq_behavior_transformer/bet.py
import tqdm
import torch
import einops
import logging
...
class BehaviorTransformer(nn.Module):
    def __init__(
        ...
        cbet_loss_multiplier: float = 1.0,
        gamma: float = 2.0,
        ...
        self._cbet_loss_multiplier = cbet_loss_multiplier
    ...
    def _maybe_fit_vq(self):
        ...
        # )
        # for epoch in pbar:
        for epoch in range(self.vqvae_iters):
    ...
    def _calc_loss(
        ...
        cbet_loss2 = self._criterion(cbet_logits_flat[:, 1], action_bins_flat[:, 1]) if cbet_logits_flat.shape[1] > 1 else 0
        ...
        equal_single_code_rate2 = eq_mask[..., 1].float().mean() if eq_mask.shape[-1] > 1 else 0.0

        try:
            loss = self._cbet_loss_multiplier * cbet_loss + self._offset_loss_multiplier * offset_loss
        except AttributeError:
            loss = cbet_loss + self._offset_loss_multiplier * offset_loss # hot fix: for old ckpts
        loss_dict = {
